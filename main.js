...
document.getElementById('start').addEventListener('click', function() {
  ...
  window.postMessage({ type: 'SCREENSHARE_REQUEST', text: 'start' }, '*');
});
...
window.addEventListener('message', function (event) {
  ...
  if (event.data.type && (event.data.type === 'SHARESCREEN_PING')) {
    extensionInstalled = true;
  }

  // user chose a stream
  if (event.data.type && (event.data.type === 'SHARESCREEN_SUCCESS')) {
    startScreenStreamFrom(event.data.streamId);
  }

  // user clicked on 'cancel' in choose media dialog
  if (event.data.type && (event.data.type === 'SHARESCREEN_CANCEL')) {
